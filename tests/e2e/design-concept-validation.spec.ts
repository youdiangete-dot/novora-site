...
test.describe('/design/start conversion flow', () => {
  test('carries design start selections into the concept brief submission', async ({ page }) => {
    ...
    const previewLink = page.getByRole('link', { name: 'View First Preview status' });
    ...
    await expect(page.getByRole('heading', { name: 'View your concept direction on the website' })).toBeVisible();
    await expect(
      page.getByText(
        'Your exact preview page will show the early AI hand-drawn concept sketch only after the automatic safety, privacy, customer-isolation, asset-validity, and lifecycle gates pass.',
      ),
    ).toBeVisible();
    await expect(
      page.getByText('Human handling is reserved for exception and correction cases.', { exact: false }),
    ).toBeVisible();
    await expect(page.getByText('not CAD, a quotation, payment confirmation', { exact: false }).first()).toBeVisible();
    await expect(page.getByText('not a manufacturability guarantee', { exact: false }).first()).toBeVisible();
    await expect(page.getByRole('link', { name: 'View demo mock preview' })).toHaveCount(0);
    await expect(previewLink).not.toHaveAttribute('href', /[?&](?:state|proof|outputId)=/);
    ...
    await expect(page.getByRole('heading', { name: 'Your automatic First Preview path' })).toBeVisible();
    await expect(
      page.getByText(
        'NOVORA may use the submitted contact information for exception or correction follow-up. This receipt and the First Preview are not CAD, a quotation, payment confirmation, an order, production approval, or a manufacturability guarantee.',
      ),
    ).toBeVisible();
    ...
    await expect(page.getByRole('heading', { name: 'Your NOVORA First Preview' })).toBeVisible();
...
test.describe('/design/preview customer-safe boundaries', () => {
  test('public-reference knowledge and query state cannot manufacture ready', async ({ page }) => {
    await page.goto('/design/preview/NOVORA-CB-20260601-LINK?state=first_preview_ready&outputId=browser-value');
    ...
    await expect(page.getByText('Not a quotation', { exact: true })).toBeVisible();
    await expect(page.getByText('Not payment confirmation', { exact: true })).toBeVisible();
    await expect(page.getByText('Not an order', { exact: true })).toBeVisible();
    await expect(page.getByText('Not production approval', { exact: true })).toBeVisible();
    await expect(page.getByText('Not a manufacturability guarantee', { exact: true })).toBeVisible();
    await expect(page.getByText('browser-value')).toHaveCount(0);
    await expect(page.getByText('Provider output')).toHaveCount(0);
    await expect(page.getByText('Prompt chain')).toHaveCount(0);
  });

  test('a valid reference without an authorized customer-view result fails safely unavailable', async ({ page }) => {
    await page.goto('/design/preview/NOVORA-CB-20260601-LINK');
  ...
  test('local and session browser state cannot authorize a ready preview', async ({ page }) => {
    await page.addInitScript(() => {
      window.localStorage.setItem('first_preview_ready', 'true');
      window.localStorage.setItem('outputId', 'browser-output');
      window.sessionStorage.setItem('first_preview_ready', 'true');
      window.sessionStorage.setItem('outputId', 'browser-output');
    });
    await page.goto('/design/preview/NOVORA-CB-20260601-LINK');

    await expect(page.getByRole('heading', { name: 'Your First Preview cannot be shown right now' })).toBeVisible();
    await expect(page.getByRole('heading', { name: 'Your First Preview is ready' })).toHaveCount(0);
    await expect(page.getByText('browser-output')).toHaveCount(0);
    await expect(page.locator('img[src^="/api/first-preview-assets/"]')).toHaveCount(0);
  });

  test('a malformed route reference fails closed without exposing an asset', async ({ page }) => {
    await page.goto('/design/preview/not-a-valid-reference?state=first_preview_ready');

    await expect(page.getByRole('heading', { name: 'This preview link is unavailable' })).toBeVisible();
    await expect(page.getByText('not-a-valid-reference')).toHaveCount(0);
    await expect(page.getByRole('heading', { name: 'Your First Preview is ready' })).toHaveCount(0);
    await expect(page.locator('img[src^="/api/first-preview-assets/"]')).toHaveCount(0);
  });

  test('the confirmed submitted receipt links only to its validated public reference', async ({ page }) => {
    ...
    const previewLink = page.getByRole('link', { name: 'View First Preview status' });
    await expect(previewLink).toBeVisible();
    await expect(previewLink).toHaveAttribute('href', '/design/preview/NOVORA-CB-20260601-LINK');
    await expect(previewLink).not.toHaveAttribute('href', /[?&](?:state|proof|outputId)=/);
    await expect(page.getByRole('link', { name: 'View demo mock preview' })).toHaveCount(0);
    await expect(page.getByText('Human handling is reserved for exception and correction cases.', { exact: false })).toBeVisible();
  });
});
...
test.describe('/design/brief submission', () => {
  ...
  test('submits valid contact fields and opens the submitted confirmation page', async ({ page }) => {
    ...
    await expect(
      page.getByText(
        'This submission helps NOVORA understand your design direction. The First Preview is an early AI hand-drawn concept sketch, not CAD, a quotation, payment confirmation, an order, production approval, or a manufacturability guarantee.',
      ),
    ).toBeVisible();
