...
type SafeJob = Readonly<{
  ...
  handSketchInstructionHash: string;
}>;
...
function mapSafeJob(value: unknown, conceptBriefId: string): SafeJob | null {
  if (
    ...
    !hasNonblankString(value.design_spec_version) ||
    typeof value.design_spec_hash !== "string" ||
    !/^[0-9a-f]{64}$/.test(value.design_spec_hash) ||
    !hasNonblankString(value.hand_sketch_instruction_version) ||
    typeof value.hand_sketch_instruction_hash !== "string" ||
    !/^[0-9a-f]{64}$/.test(value.hand_sketch_instruction_hash) ||
    ![
      "queued",
      "processing",
      "succeeded",
  ...
  return {
    ...
    completedAt,
    failedAt,
    designSpecVersion: value.design_spec_version,
    designSpecHash: value.design_spec_hash,
    handSketchInstructionVersion: value.hand_sketch_instruction_version,
    handSketchInstructionHash: value.hand_sketch_instruction_hash,
  };
}
...
function mapSafeOutput(
  ...
  if (
    ...
    value.concept_brief_id !== conceptBriefId ||
    value.bucket_name !== FIRST_PREVIEW_ASSET_BUCKET ||
    value.mime_type !== "image/png" ||
    readPositiveInteger(value.byte_size) === null ||
    Number(value.byte_size) > FIRST_PREVIEW_GENERATED_ASSET_MAX_BYTES ||
    value.width_px !== 1024 ||
    value.height_px !== 1024 ||
    typeof value.content_sha256 !== "string" ||
    !isValidFirstPreviewContentSha256(value.content_sha256) ||
    value.object_path !==
      deriveFirstPreviewGeneratedAssetId({
        conceptBriefId,
        jobId: value.job_id,
    ...
    !["pending", "passed", "failed"].includes(
      value.asset_validation_status as string,
    ) ||
...
function hasValidCandidateLineage(
  ...
  if (orderedJobs.length === 2) {
    ...
    if (
      ...
      second.designSpecVersion !== first.designSpecVersion ||
      second.designSpecHash !== first.designSpecHash ||
      second.handSketchInstructionVersion !==
        first.handSketchInstructionVersion ||
      second.handSketchInstructionHash !==
        first.handSketchInstructionHash ||
      first.status !== "failed" ||
