...
export const FIRST_PREVIEW_CUSTOMER_DESIGN_CONFIRMATION_VERSION =
  "customer_design_direction_confirmation_v1" as const;
export const FIRST_PREVIEW_CUSTOMER_DESIGN_CONFIRMATION_BINDING_MAX_TOKEN_BYTES =
  1_024 as const;

const BINDING_VERSION = 1 as const;
const BINDING_ALGORITHM = "HS256" as const;
const BINDING_AUDIENCE =
  "novora:first-preview-design-direction-confirmation" as const;
const BINDING_PURPOSE = "selected-design-direction-confirmation" as const;
const BINDING_SIGNING_DOMAIN =
  "NOVORA\0first-preview-design-direction-confirmation-binding\0v1\0hmac-sha-256\0";
const BASE64URL_PATTERN = /^[A-Za-z0-9_-]+$/;
const MINIMUM_SIGNING_SECRET_BYTES = 32;
const MAXIMUM_SIGNING_SECRET_RAW_CHARACTERS = 4_096;

export type FirstPreviewCustomerDesignConfirmationBindingClaims = Readonly<{
...
export type FirstPreviewCustomerDesignConfirmationWriteResult =
  | Readonly<{ ok: true; status: "created" | "already_confirmed" }>
  | Readonly<{ ok: false; reason: "invalid" | "denied" | "unavailable" }>;

export interface FirstPreviewCustomerDesignConfirmationRepository {
  resolveExactCurrentPair(
    publicReference: string,
    outputId: string,
  ): Promise<Readonly<{ conceptBriefId: string; outputId: string }> | null>;
  insertExactConfirmation(input: Readonly<{
    conceptBriefId: string;
    outputId: string;
    confirmationVersion: typeof FIRST_PREVIEW_CUSTOMER_DESIGN_CONFIRMATION_VERSION;
...
type Dependencies = Readonly<{
  ...
  repository: FirstPreviewCustomerDesignConfirmationRepository;
...
function hasExactBindingClaims(
  value: Record<string, unknown>,
): value is FirstPreviewCustomerDesignConfirmationBindingClaims {
  return (
    hasExactKeys(value, [
      "alg",
      "aud",
    ...
    value.v === BINDING_VERSION &&
    value.alg === BINDING_ALGORITHM &&
    value.aud === BINDING_AUDIENCE &&
    value.purpose === BINDING_PURPOSE &&
...
export function createFirstPreviewCustomerDesignConfirmationBinding(
  input: Readonly<{ publicReference: string; outputId: string }>,
  signingSecret: string,
): string | null {
  ...
  const claims: FirstPreviewCustomerDesignConfirmationBindingClaims = {
    v: BINDING_VERSION,
  ...
  return Buffer.byteLength(token, "utf8") <=
    FIRST_PREVIEW_CUSTOMER_DESIGN_CONFIRMATION_BINDING_MAX_TOKEN_BYTES
    ? token
    : null;
}

export function verifyFirstPreviewCustomerDesignConfirmationBinding(
  token: unknown,
  signingSecret: unknown,
): FirstPreviewCustomerDesignConfirmationBindingClaims | null {
  if (
    typeof token !== "string" ||
  ...
  const expectedSignature = signBinding(encodedPayload, signingSecret);
  if (
  ...
  if (!claims || !hasExactBindingClaims(claims)) return null;
...
export function normalizeFirstPreviewCustomerDesignConfirmationBody(
  body: unknown,
...
export function createSupabaseFirstPreviewCustomerDesignConfirmationRepository(
  supabase: SupabaseClient,
): FirstPreviewCustomerDesignConfirmationRepository {
  return {
    async resolveExactCurrentPair(publicReference, outputId) {
      try {
        const { data: briefs, error: briefError } = await supabase
          ...
          .limit(2);
        if (briefError || !Array.isArray(briefs) || briefs.length !== 1) {
          return null;
        }
        ...
        const { data: outputs, error: outputError } = await supabase
          ...
          .select(
            "id, concept_brief_id, readiness_status, is_current_customer_preview, readiness_revoked_at",
          )
          .eq("id", outputId)
          ...
          .limit(2);
        if (outputError || !Array.isArray(outputs) || outputs.length !== 1) {
          return null;
        }
...
function exactReadyOutputFromView(
  customerView: FirstPreviewCustomerView,
  publicReference: string,
  signedOutputId: string,
): "exact" | "denied" | "unavailable" {
  if (customerView.state === "denied") return "denied";
  if (customerView.state !== "ready") return "unavailable";
  if (
  ...
  return customerView.assetRequest.outputId === signedOutputId
    ? "exact"
    : "unavailable";
}

export async function persistFirstPreviewCustomerDesignConfirmation(
  publicReference: unknown,
  body: unknown,
  dependencies?: Dependencies,
): Promise<FirstPreviewCustomerDesignConfirmationWriteResult> {
  if (
  ...
  const normalizedBody =
    normalizeFirstPreviewCustomerDesignConfirmationBody(body);
  ...
  const signingSecret =
    ...
    null;
  if (!isUsableSigningSecret(signingSecret)) {
    return { ok: false, reason: "unavailable" };
  }
  const signedBinding =
    verifyFirstPreviewCustomerDesignConfirmationBinding(
      normalizedBody.binding,
      signingSecret,
    );
  if (!signedBinding) return { ok: false, reason: "invalid" };
  if (signedBinding.publicReference !== publicReference) {
  ...
  const activeDependencies = dependencies ?? {
    ...
    repository:
      createSupabaseFirstPreviewCustomerDesignConfirmationRepository(supabase!),
    signingSecret,
  };

  try {
    const firstView = await activeDependencies.readCustomerView({
      publicReference,
    });
    ...
    if (
      !pair ||
      pair.outputId !== signedBinding.outputId ||
      !isValidFirstPreviewAssetUuid(pair.conceptBriefId)
    ) {
      return { ok: false, reason: "unavailable" };
    }

    const finalView = await activeDependencies.readCustomerView({
      publicReference,
    });
    ...
    const inserted =
      await activeDependencies.repository.insertExactConfirmation({
        conceptBriefId: pair.conceptBriefId,
        outputId: pair.outputId,
        confirmationVersion:
          FIRST_PREVIEW_CUSTOMER_DESIGN_CONFIRMATION_VERSION,
      });
